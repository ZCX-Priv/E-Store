...
function loadSelectedCategoryId(): string | number {
  const stored = safeGetItem('inventory-selected-category')
...
export const useUiStore = defineStore('ui', () => {
  ...
  const viewMode = ref<ViewMode>(
    (safeGetItem('inventory-view-mode') as ViewMode | null) || 'grid'
  )

  // 侧边栏是否收起（桌面端）
  const sidebarCollapsed = ref<boolean>(
    safeGetItem('inventory-sidebar-collapsed') === 'true'
  )
  ...
  const theme = ref<ThemeMode>(
    (safeGetItem('inventory-theme') as ThemeMode | null) || 'system'
  )
  ...
  function setViewMode(mode: ViewMode) {
    viewMode.value = mode
    safeSetItem('inventory-view-mode', mode)
  }

  // 切换侧边栏收起状态并持久化
  function toggleSidebar() {
    sidebarCollapsed.value = !sidebarCollapsed.value
    safeSetItem('inventory-sidebar-collapsed', String(sidebarCollapsed.value))
  }

  // 设置侧边栏收起状态
  function setSidebarCollapsed(collapsed: boolean) {
    sidebarCollapsed.value = collapsed
    safeSetItem('inventory-sidebar-collapsed', String(collapsed))
  }

  // 选中分类并持久化
  function selectCategory(id: string | number) {
    selectedCategoryId.value = id
    safeSetItem('inventory-selected-category', String(id))
  }
  ...
  function setTheme(mode: ThemeMode) {
    theme.value = mode
    safeSetItem('inventory-theme', mode)
  }
